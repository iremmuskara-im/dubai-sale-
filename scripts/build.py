...
def get_sample_items():
    return [
        ...
        {"name":"King Bed Frame","description":"Solid walnut wood frame","price":"1200","category":"bedroom","status":"available","emoji":"🛏️","photos":[]},
    ]
...
def render_item_card(item, wa_number, item_id):
    status = item["status"]
    ...
    photos_json = str(item["photos"]).replace("'", '"')
...
<style>
  ...
  *{{margin:0;padding:0;box-sizing:border-box}}
  ...
  .cart-btn{{display:flex;align-items:center;gap:8px;padding:10px 20px;background:var(--brown);color:white;border:none;border-radius:2px;font-family:'Jost',sans-serif;font-size:12px;letter-spacing:2px;text-transform:uppercase;cursor:pointer;transition:all .2s;position:relative}}
  ...
  .cart-wa-btn,.cart-email-btn{{padding:14px;border-radius:2px;font-family:'Jost',sans-serif;font-size:12px;letter-spacing:2px;text-transform:uppercase;cursor:pointer;border:none;transition:all .2s;text-align:center;text-decoration:none;display:block}}
  .cart-wa-btn{{background:#25d366;color:white}}
...
function renderCart(){{
  ...
  // Build WA & Email links
  let msg='Hi! I am interested in the following items from your Dubai sale:\\n\\n';
  let emailBody='Hi,\\n\\nI am interested in the following items from your Dubai sale:\\n\\n';
  cart.forEach((item,i)=>{{
